...
namespace TripMaker
{
    public partial class FlightManagerView : MetroFramework.Forms.MetroForm
    {
        ...
        private void loadgrid()
        {
            ...
            try
            {
                string query = @"SELECT Transport_Id AS flight_id,
                                Transport_Name AS flight_company,
        ...
        private void loaddetail(string id)
        {
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    txtid.Text = dt.Rows[0]["flight_id"].ToString();
                    txtName.Text = dt.Rows[0]["flight_name"].ToString();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (string.IsNullOrEmpty(id))
                {
                    ...
                    VALUES (seq_Transport.NEXTVAL, '{fname}', 'Flight', {bt},
                            TO_DATE('{dt}', 'YYYY-MM-DD HH24:MI'),
                            TO_DATE('{at}', 'YYYY-MM-DD HH24:MI'),
                            {price}, (SELECT Admin_Id FROM Admin WHERE Admin_Username = '{Session.LoggedInUsername}'), '{fl}', '{tl}')";
